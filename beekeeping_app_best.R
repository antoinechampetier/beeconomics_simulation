...
plot_stock_dynamics <- function(results_df, panels_to_show = c("colonies", "frames", "frames_per_colony", "forage_balance")) {
  ...
  facet_labels <- c(
    ...
    "forage_balance" = "Forage/Feed/Honey Flows"
  )
                  ...
                  div(
                    ...
                    fluidRow(
                      ...
                      column(
                        3,
                        h6("Crop Pollination"),
                        sliderInput("D_spring", "Crop Max Value:", value = 4, min = 0, max = 100, step = 0.5),
                  ...
                  div(
                    ...
                    fluidRow(
                      ...
                      column(
                        ...
                        sliderInput("B_summer", "Forage Midpoint:", value = 22000, min = 0, max = 100000, step = 1000),
                  ...
                  div(
                    ...
                    fluidRow(
                      ...
                      column(
                        3,
                        h6("Crop Pollination"),
                        sliderInput("D_winter", "Crop Max Value:", value = 15.5, min = 0, max = 20, step = 0.5),
                        sliderInput("E_winter", "Crop Midpoint:", value = 83000, min = 0, max = 100000, step = 1000),
                        sliderInput("Pc_winter", "Crop Price ($/lbs):", value = 2, min = 0, max = 100, step = 1),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$optimize_mgmt, {
    ...
    # Create schedule and locations
    schedule <- create_simple_schedule(input$n_years %||% 3, input$start_season %||% "Spring")
    locations <- create_location_parameters(schedule, params)
  ...
  # Steady-state cycle optimization
  observeEvent(input$find_steady_cycle, {
    req(input$initial_colonies, input$initial_frames, input$P_feed)
    
    showNotification("Finding steady-state cycle... This may take a few minutes.", type = "message", duration = 3)
    ...
    initial_params <- c(
      ...
      input$cull_fall %||% 0,
      input$cull_winter %||% 0,
      input$adjust_spring %||% 0,
      input$adjust_summer %||% 0,
      input$adjust_fall %||% 0,
      input$adjust_winter %||% 0
    )
    ...
    tryCatch(
      {
        opt_result <- optim(
          par = initial_params,
          fn = function(params) {
            optimize_steady_cycle(
              params, locations,
              input$initial_colonies, input$initial_frames,
              input$P_feed, management_costs
            )
          },
          method = "L-BFGS-B",
          lower = c(-100, -100, -100, -100, -100, -100, -100, -100),
          upper = c(100, 100, 100, 100, 100, 100, 100, 100),
          control = list(maxit = 200)
        )
        
        # Update UI with optimized values
        updateNumericInput(session, "cull_spring", value = round(opt_result$par[1], 1))
        updateNumericInput(session, "cull_summer", value = round(opt_result$par[2], 1))
        updateNumericInput(session, "cull_fall", value = round(opt_result$par[3], 1))
        updateNumericInput(session, "cull_winter", value = round(opt_result$par[4], 1))
        updateNumericInput(session, "adjust_spring", value = round(opt_result$par[5], 1))
        updateNumericInput(session, "adjust_summer", value = round(opt_result$par[6], 1))
        updateNumericInput(session, "adjust_fall", value = round(opt_result$par[7], 1))
        updateNumericInput(session, "adjust_winter", value = round(opt_result$par[8], 1))
        
        # Calculate CV for display
        final_cv <- opt_result$value
        cv_message <- sprintf("Steady cycle found! Total CV: %.4f", final_cv)
        
        showNotification(cv_message, type = "message", duration = 10)
      },
      error = function(e) {
        showNotification(
  ...
  # Optimize management for selected period range
  observeEvent(input$optimize_period, {
    # Show progress
    showNotification("Starting period-based optimization... This may take a few minutes.", type = "message", duration = 3)

    # Get selected period range
    if (is.null(input$period_range)) {
      showNotification("No period range selected. Using all periods.", type = "warning", duration = 5)
      results_df <- simulation_results()
      if (is.null(results_df)) {
        showNotification("No simulation data available. Cannot optimize.", type = "error", duration = 5)
        return()
      }
      start_period <- 1
      end_period <- nrow(results_df)
    } else {
      start_period <- input$period_range[1]
      end_period <- input$period_range[2]
    }
    ...
    # Run optimization for selected period
    tryCatch(
      {
        cat("\n=== PERIOD-BASED OPTIMIZATION ===\n")
        cat("Optimizing for periods", start_period, "to", end_period, "\n")
        cat("Starting parameters:", paste(start_params, collapse = ", "), "\n")
        ...
        opt_result <- optim(
          ...
          fn = function(params) {
            ...
            tryCatch(
              {
                result <- optimize_management_period(
                  params, locations, input$initial_colonies %||% 1000,
                  input$initial_frames %||% 6000, input$P_feed %||% 2.0,
                  mgmt_costs, start_period, end_period
                )
                if (!is.finite(result)) {
                  cat("WARNING: Non-finite result:", result, "\n")
                  return(1e10)
                }
                return(result)
              },
        ...
        showNotification(
          paste0(
            "Period optimization completed! ",
            "Optimal profit for periods ", start_period, "-", end_period, ": $",
            format(round(-opt_result$value), big.mark = ",")
          ),
          type = "message", duration = 10
        )
      },
      error = function(e) {
        showNotification(paste("Period optimization failed:", e$message), type = "error", duration = 10)
      }
    )
  })
  ...
  output$save_settings <- downloadHandler(
    ...
    content = function(file) {
      ...
      units <- c(
        ...
        # Seasonal biological parameters
        rep("frames/colony/week", 4),
        rep("proportion/season", 4),
        rep("lbs/frame/week", 4),
        rep("unitless", 8),
        # Economic parameters
        "$/colony/year", "$/lb", "$/operation", "$/operation", "$/operation",
        # Management parameters
        rep("%", 8),
        # Seasonal forage production parameters
        rep("lbs/forager", 4),
    ...
    if (nrow(results_df) > 0) {
      ...
      p <- p + geom_point(
        ...
        annotate(
          ...
          y = start_point$frames_start,
          label = "START",
          nudge_y = max(results_df$frames_end_mgmt, na.rm = TRUE) * 0.03,
          color = "red",
          fontface = "bold",
          size = 4
        )
